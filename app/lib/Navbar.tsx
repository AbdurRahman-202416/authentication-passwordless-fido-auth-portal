...
// Mocking Link component for single-file usability
...
// Inline SVG for a Key Icon (Lucide Key equivalent)
// Defined with an explicit type for the props, even in a JSX file, for clarity
...
export default function Navbar() {
  // Use state to simulate the current active path. Initialize to Home ('/')
  const [activePath, setActivePath] = useState("");

  // Helper function to dynamically generate classes based on the active path
  const getLinkClasses = (href, isButton = false) => {
    const isActive = activePath === href;

    if (isButton) {
      // Classes for the highlighted Sign In button
      return isActive
    ...
    // Classes for regular text links (Home, Register)
    return isActive
  ...
  // Update the path state on click
  const handleLinkClick = (e, href) => {
   
    setActivePath(href);
    // In a real Next.js app, you'd use router.push(href) here.
  };
    ...
    <nav className="bg-gray-900 shadow-xl border-b border-indigo-500/30 sticky top-0 z-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between gap-10 items-center h-16">
          ...
          <Link
            href="/"
            className="flex items-center space-x-3 text-white transition duration-300 hover:text-indigo-400"
          ...
          <div className="flex items-center space-x-6 text-sm sm:text-base">
            ...
            {/* Highlighted Login/Sign In button */}
            <Link
              href="/login"
